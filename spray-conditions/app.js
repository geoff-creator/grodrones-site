const APP_VERSION = "spray-conditions-debug-v2";
...
const rawNoaaNote = document.getElementById("rawNoaaNote");
...
map.on("click", async (event) => {
  ...
  if (!isWithinSprayRegion(lat, lng)) {
    ...
    setRawNoaaNote("Raw NOAA hourly values: outside intended region.");
    return;
  }
  ...
  setRawNoaaNote("Raw NOAA hourly values: loading...");
  ...
  try {
    ...
    const period = selectCurrentHourlyPeriod(hourlyData?.properties?.periods, Date.now());
    if (!period) throw new Error("No hourly forecast period available.");

    const windSpeedRaw = period.windSpeed ?? null;
    const windGustRaw = period.windGust ?? null;

    console.log("NOAA hourly raw values", {
      periodName: period.name,
      startTime: period.startTime,
      windSpeed: windSpeedRaw,
      windGust: windGustRaw,
      windDirection: period.windDirection,
    });

    setRawNoaaNote(
      `Raw NOAA hourly values — period: ${period.name || "(unnamed)"}; startTime: ${period.startTime || "n/a"}; ` +
        `windSpeed: ${windSpeedRaw ?? "n/a"}; windGust: ${windGustRaw ?? "n/a"}; windDirection: ${period.windDirection || "n/a"}`,
    );

    const periodStart = new Date(period.startTime || Date.now());
    const windSpeedMph = parseWindSpeed(windSpeedRaw);
    const parsedWindGustMph = parseGustSpeed(windGustRaw);
    const windGustMph = sanitizeWindGust(parsedWindGustMph, windSpeedMph);
    ...
    const sprayStatus = determineSprayStatus({ windSpeedMph, windGustMph, rainProb, deltaTF });
    ...
    setSourceNote(
      `Debug source: period selected by current time window from forecastHourly.periods[*]. ` +
        `wind speed = period.windSpeed (${hourlyUrl}); wind gust = period.windGust (${windGustMph == null ? "unavailable (missing or failed sanity check)" : "used"}).`,
    );
    ...
    setRawNoaaNote("Raw NOAA hourly values: unavailable due to fetch error.");
...
function selectCurrentHourlyPeriod(periods, nowMs) {
  ...
  const current = parsed.find((entry) => nowMs >= entry.startMs && nowMs < entry.endMs);
  if (current) return current.period;

  const future = parsed.find((entry) => entry.startMs > nowMs);
  if (future) return future.period;

  return parsed.at(-1)?.period ?? null;
}
...
function setRawNoaaNote(text) {
  rawNoaaNote.textContent = text;
}
...
function sanitizeWindGust(windGustMph, windSpeedMph) {
  if (windGustMph == null) return null;
  if (windSpeedMph == null) return null;
  if (windGustMph > windSpeedMph * 3) return null;
  return windGustMph;
}
...
function determineSprayStatus({ windSpeedMph, windGustMph, rainProb, deltaTF }) {
  ...
  const gustSuspicious = hasWind && hasGust && windGustMph >= windSpeedMph * 3 && windGustMph >= 20;
  if (gustSuspicious) {
    return { level: "yellow", message: "Possible data anomaly", confidence };
  }
