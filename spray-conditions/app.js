...
map.on("click", async (event) => {
  ...
  try {
    ...
    const windGustMph = parseGustSpeed(windGustRaw);
    ...
    updateDetail("Wind Gust", windGustRaw == null || String(windGustRaw).trim() === "" ? "unavailable" : `${windGustRaw} (raw), ${windGustMph == null ? "unparsed" : `${windGustMph.toFixed(1)} mph`}`);
    ...
    setSourceNote(
      `Debug source: period selected by current time window from forecastHourly.periods[*]. ` +
        `wind speed = period.windSpeed (${hourlyUrl}); wind gust = period.windGust (${windGustMph == null ? "unavailable" : "used"}).`,
    );
